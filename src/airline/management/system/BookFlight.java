...
public class BookFlight extends JFrame implements ActionListener{
    ...
    public BookFlight() {
        ...
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("airline/icons/details.jpg"));
...
public class BookFlight extends JFrame implements ActionListener {
    ...
    public BookFlight() {
        ...
        JLabel heading = new JLabel("Book Flight", SwingConstants.CENTER);
        ...
        String[] labels = {"Aadhar", "Name", "Nationality", "Address", "Gender", "Source", "Destination", "Flight Name", "Flight Code", "Date of Travel"};
        int y = 120;
        ...
        fetchButton = new JButton("Fetch User");
        fetchButton.setBounds(380, 120, 120, 25);
        ...
        tfaddress = new JLabel();
        tfaddress.setBounds(220, 240, 200, 25);
        add(tfaddress);

        labelgender = new JLabel();
        labelgender.setBounds(220, 280, 200, 25);
        add(labelgender);

        source = new Choice();
        source.setBounds(220, 320, 150, 25);
        add(source);
        ...
        flight = new JButton("Fetch Flights");
        flight.setBounds(380, 360, 120, 25);
        styleButton(flight);
        flight.addActionListener(this);
        add(flight);

        labelfname = new JLabel();
        labelfname.setBounds(220, 400, 200, 25);
        add(labelfname);

        labelfcode = new JLabel();
        labelfcode.setBounds(220, 440, 200, 25);
        add(labelfcode);
        ...
        lblimage.setBounds(550, 120, 500, 380);
        add(lblimage);

        bookflight = new JButton("Book Flight");
        bookflight.setBounds(220, 530, 150, 30);
        styleButton(bookflight);
        bookflight.addActionListener(this);
        add(bookflight);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == fetchButton) {
            try {
                Conn conn = new Conn();
                ResultSet rs = conn.s.executeQuery("select * from passenger where aadhar = '" + tfaadhar.getText() + "'");
                if (rs.next()) {
                    tfname.setText(rs.getString("name"));
                    tfnationality.setText(rs.getString("nationality"));
                    tfaddress.setText(rs.getString("address"));
            ...
            try {
                Conn conn = new Conn();
                ResultSet rs = conn.s.executeQuery("select * from flight where source = '" + source.getSelectedItem() + "' and destination = '" + destination.getSelectedItem() + "'");
                if (rs.next()) {
                    labelfname.setText(rs.getString("f_name"));
                    labelfcode.setText(rs.getString("f_code"));
                } else {
            ...
            Random random = new Random();
            String query = String.format("insert into reservation values('PNR-%d', 'TIC-%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
